...
export function InvestorPitchVideo({ isDark, onClose }: InvestorPitchVideoProps) {
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      const scaleY = (window.innerHeight - 100) / baseHeight; // Leave room for controls
  ...
  useEffect(() => {
    const slideIndex = SLIDE_TIMINGS.findIndex(
      (timing, i) => currentTime >= timing.start && currentTime < timing.end
    );
    ...
    <div className="w-full h-screen bg-black flex flex-col items-center overflow-hidden">
      ...
      <div className="w-full max-w-5xl px-8 py-4 bg-slate-900/90 backdrop-blur-md border-t border-cyan-500/20">
        ...
        <div className="flex items-center justify-between">
          ...
          {/* Download hint */}
          <div className="flex items-center gap-4">
            <span className="text-slate-500 text-sm">Space to play/pause • Arrows to navigate</span>
            {onClose && (
              <button
                onClick={onClose}
